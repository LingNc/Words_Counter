...
void ChainHash::resize(size_t newsize){
    ...
    for (size_t i = 0; i < _hash.size(); i++){
        ...
        if (cur.get_head()){
            std::shared_ptr<Node<std::string>> tmp = cur.get_head();
            while (tmp){
                std::string word = tmp->data;
                size_t freq = tmp->frequency;

                uint32_t hashvalue = fnv1a_hash(word) % newsize;
                std::shared_ptr<Node<std::string>> new_node = newhash[hashvalue].push_back(word);
...
const FreqTable& ChainHash::get_frequency_table() const {
    ...
    for (size_t i = 0; i < _hash.size(); i++){
        std::shared_ptr<Node<std::string>> current = _hash[i].get_head();
...
ChainHash::Ret ChainHash::_search_word(const std::string &word) {
    ...
    Pair<std::shared_ptr<Node<std::string>>, size_t> node = _hash[hashvalue].find(word);
